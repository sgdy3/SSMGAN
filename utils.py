...
def mask_generator_batch(shadow, shadow_free):
    mask=[]
    for i in range(shadow.shape[0]):
        im_f=(to_gray(to_pil(((shadow_free[i,:,:,:].data.squeeze(0) + 1.0) * 0.5).cpu())))
        im_s=(to_gray(to_pil(((shadow.data[i,:,:,:].squeeze(0) + 1.0) * 0.5).cpu())))

        diff = (np.asarray(im_f, dtype='float32')- np.asarray(im_s, dtype='float32')) # difference between shadow image and shadow_free image
        L = threshold_otsu(diff)
        mask.append(torch.tensor((np.float32(diff >= L))).unsqueeze(0).unsqueeze(0).cuda()) # 0:non-shadow, 1.0:shadow
...
class LambdaLR():
    def __init__(self, n_epochs, offset, decay_start_epoch):
        ...
        lr reachs the minimal value " lr_max*(offset)/(n_epochs-decay_start_epoch)" when epoch=n_epochs
        :param n_epochs: number of epochs
        :param offset: intercept to avoid lr=0 when training ends
        :param decay_start_epoch: epoch when lr start decaying
        '''
        assert ((n_epochs - decay_start_epoch) > 0), "Decay must start before the training session ends!"
        assert offset>=0, "offset epoch can not less than 0"
        self.n_epochs = n_epochs
        self.offset = offset
